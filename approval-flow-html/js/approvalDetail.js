layui.use('form', function() {
    ...
    function onLoad() {
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				initRole(data.data.roleSetEntities);
	...
	function initRole(roleSetEntities) {
		var html = "<table><tr>";
		
		$.each(roleSetEntities,function (index,obj) {
			html += "<td><input type='checkbox' name='exe-role' value='" + obj.roleCode + "'  title='" + obj.roleName +"' checked></td>";
			if ((index + 1) % 8 == 0){
				html += "</tr><tr>";
			}
		});
		...
		html += "</tr></table>";
		$("#roleCheckboxDiv").append(html);
		$("#receiveMsgRoleCheckboxDiv").append(html);
	}
	...
	function cleanNodeDetailDiv() {
		...
		$("#nodeDetailDiv input[type='checkbox']").removeAttr("checked");
	}
